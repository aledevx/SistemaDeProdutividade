...
public partial class MinhasProdPage : ComponentBase
{
    #region Properties
    ...
    protected override async Task OnInitializedAsync()
    {
        ...
        try
        {
            ...
            var result = await prodHandler.MinhasProdutividadesAsync(usuarioLogado!.Cpf);
            if (result.IsSuccess)
            {
    ...
    public bool FilterFunc(ProdFeitaIndexVM prod, string searchString)
    {
        if (string.IsNullOrWhiteSpace(searchString))
            return true;
        if (prod.Lotacao.Contains(searchString, StringComparison.OrdinalIgnoreCase))
            return true;
