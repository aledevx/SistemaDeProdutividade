...
namespace SistemaDeProdutividade.Application.UseCases.Produtividade.VisualizarRecebidas;
public class VisualizarProdutividadesRecebidasUseCase : IVisualizarProdutividadesRecebidasUseCase
{
    ...
    public VisualizarProdutividadesRecebidasUseCase(IUsuarioReadOnlyRepository userReadOnlyRepository, 
    ...
    public async Task<ProdsFeitasResponseJson> Execute(string cpfUsuarioLogado)
    {
        ...
        {
            var result = prodsRecebidas.Prods.Where(p => p.Status == Domain.Enums.StatusProdutividade.AguardandoRh).Select(p => new ProdFeitaIndexVM(p.Id,
            0,
            p.Codigo,
            p.NomeUsuario,
            p.Cargo,
